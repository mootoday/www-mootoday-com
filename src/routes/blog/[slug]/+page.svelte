<script lang="ts">
  ...
  const datePublishedDisplay = new Date(Date.parse(data.metadata?.createdAt)).toLocaleDateString(undefined, {
    dateStyle: "long"
  });

  const datePublishedYYYYMMDD = new Date(Date.parse(data.metadata?.createdAt)).toLocaleDateString(undefined, {
    dateStyle: "short"
  });
</script>
